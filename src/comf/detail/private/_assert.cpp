...
#if !defined(app_assert_log_tag)
#define app_assert_log_tag "app_assert"
#endif
#endif

//#define app_assert_log_file "/tmp/assert.txt"
...
// app_assert_exception_message_buffer_size == app_assert_message_buffer_size
// which is the default.
#if !defined(app_assert_malloc)
#define app_assert_malloc(size) malloc(size)
#endif

#if !defined(app_assert_free)
#define app_assert_free(p) free(p)
#endif

#if !defined(app_assert_message_buffer_size)
#  define app_assert_message_buffer_size app_assert_exception_message_buffer_size
#endif

#if !defined(app_assert_abort)
#define app_assert_abort abort
#endif
...
namespace {
...
#if defined(app_assert_log_file) && defined(app_assert_log_file_truncate)
  struct LogFileTruncate
  {
    LogFileTruncate()
    {
      if (FILE* f = fopen(app_assert_log_file, "w"))
        fclose(f);
    }
  };
  ...
  int print(FILE* out, int level, const char* format, ...)
  {
...
#if defined(app_assert_log_file)
    struct Local
    {
      static void log(const char* _format, va_list _args)
      {
        if (FILE* f = fopen(app_assert_log_file, "a"))
        {
...
#if defined(_WIN32)
    char buffer[app_assert_message_buffer_size];
    va_start(args, format);
    vsnprintf(buffer, app_assert_message_buffer_size, format, args);
    ...
    va_start(args, format);
    __android_log_vprint(priority, app_assert_log_tag, format, args);
    va_start(args, format);
#else
    app_assert_unused(level);
#endif
  ...
  AssertAction::AssertAction app_assert_call _defaultHandler( const char* file,
                                                              int line,
                                                              const char* function,
                                                              ...
                                                              const char* message)
  {
    ...
    {
#if (!TARGET_OS_IPHONE && !TARGET_IPHONE_SIMULATOR) && (!defined(__ANDROID__) && !defined(ANDROID)) || defined(app_assert_default_handler_stdin)
      for (;;)
      {
...
namespace assert {
  ...
  AssertionException::AssertionException(const char* file,
                                         ...
                                         const char* expression,
                                         const char* message)
  : _file(file), _line(line), _function(function), _expression(expression), _heap(app_assert_nullptr)
  {
    if (!message)
    {
    ...
    {
      _heap = static_cast<char*>(app_assert_malloc(sizeof(char) * (length + 1)));
  ...
  AssertionException::~AssertionException() app_assert_exception_no_throw
  {
    if (_stack[size - 1])
      app_assert_free(_heap);

    _heap = app_assert_nullptr; // in case the exception object is destroyed twice
    _stack[size - 1] = 0;
  }
  ...
  const char* AssertionException::what() const app_assert_exception_no_throw
  {
...
namespace implementation {
  ...
  AssertAction::AssertAction app_assert_call handleAssert(const char* file,
                                                          ...
                                                          const char* message, ...)
  {
    char message_[app_assert_message_buffer_size] = {0};
    const char* file_;
    ...
    if (message)
    {
      ...
      vsnprintf(message_, app_assert_message_buffer_size, message, args);
      va_end(args);
...
#if defined(app_assert_default_action)
    AssertAction::AssertAction action = app_assert_default_action;
#else
